...
The owner can list configured projects and add any accessible GitHub repository as a new project. The repository field accepts either a name such as `Zekhet` or a full URL such as `https://github.com/daesaesthetic/Zekhet`. Project-specific commands accept either the returned project ID or the project display name, ignoring capitalization. If two projects share a name, Discord asks for the project ID instead.
...
Project-owned schema sources declare foreign-key relationships to `projects` for Context, Reality, milestones, and GitHub repository associations, with cascading cleanup where appropriate. The development database must be inspected for orphaned rows before applying these constraints. Replit-managed PostgreSQL schema changes should flow through the development post-merge process and Publish; the application does not run a custom migration runner or mutate production schema at startup.
...
- `/explain project` authorizes through `ProjectService`, selects only bounded Intelligence evidence, enforces the returned project ID, redacts sensitive structures and values, and sends the package to the local `MockAiService`.
...
- `MockAiService` is deterministic, local, provider-neutral, credential-free, and network-free. It formats only supplied evidence and explicitly supplied unavailable information; it consumes no AI/API credits. A real provider must be explicitly selected and connected before replacing it.
...
The current regression suite contains 80 passing tests.
